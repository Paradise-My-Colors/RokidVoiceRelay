import wx from 'wx';
import { NetworkBridge, identifier, timeout, errorMessage } from './network.js';
import { keyAction, visibleRows, excerpt, receiptTitle, recordingMime } from '../voice-relay/lib/ui.js';
const PHONE_PROFILE = "__VOICE_RELAY_PHONE_PROFILE__";
...
export default {
  ...
  runtimeCheck() {
    let runtime = 'Unknown';
    try { runtime = navigator.userAgent || (navigator.versions && navigator.versions.ink) || runtime; } catch (_) {}
    const d = 'Network: ' + (typeof wx.request === 'function' ? 'yes' : 'no') + ' · Recorder: ' + (typeof MediaRecorder !== 'undefined' ? 'yes' : 'no') + ' · Playback: ' + (typeof AudioPlayer !== 'undefined' ? 'yes' : 'no');
  ...
  async playAudio(data, mime, returnScreen) {
    ...
    if (mime.includes('wav')) {
      ...
      const context = new AudioContext(); let source;
      const player = { stop() { if (source) { try { source.stop(); } catch (_) {} } }, destroy() { try { const closed = context.close(); if (closed && closed.catch) closed.catch(() => {}); } catch (_) {} } };
      this.player = player;
      try {
        const buffer = await context.decodeAudioData(data);
        if (this.player !== player || !this.visible) return;
        source = context.createBufferSource(); source.buffer = buffer; source.connect(context.destination);
        source.onended = () => { if (this.player !== player) return; this.stopPlayer(); if (returnScreen === 'preview') this.preview(); else this.detail(); };
        await context.resume();
        if (this.player !== player || !this.visible) return;
        source.start(); this.show('playing', 'Playing', this.current.sender, [row('stopPlay', 'Stop playback')]); return;
      } catch (e) { if (this.player === player) this.stopPlayer(); throw e; }
    }
    ...
    player.setBuffer(data, mime.includes('ogg') ? 'ogg' : 'wav'); player.play();
