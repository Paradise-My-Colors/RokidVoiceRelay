...
1. **On the Android phone:** install `RokidVoiceRelay-v0.9-AIUI-test.apk` over the existing Voice Relay installation. Keep the existing installation so its Telegram login and saved settings remain available. If Android reports a conflicting signature, stop instead of uninstalling; the build checks its signer against the published v0.8 APK.
2. Open **Voice Relay AIUI** on the phone. Tap **1. Enable notification access** and enable Voice Relay in Android's list. Return and tap **2. Start Bluetooth bridge**. Allow Nearby devices and notifications. Leave Bluetooth enabled.
3. Open **Telegram setup / login**. If already connected, keep that session. Otherwise enter your own Telegram API credentials and complete login on the phone using the same Telegram account that receives the notifications. These credentials are not included in the project.
...
| Listen on the glasses | Downloads a selected voice note through the logged-in Telegram client | Uses accessible notification audio; otherwise share the chosen voice message from WhatsApp to **Voice Relay** on the phone and select the matching conversation |
...
The build compiles the Android APK, verifies its signature and compares its signing certificate to published v0.8. The branch is [codex/aiui-voice-relay-0.9](https://github.com/Paradise-My-Colors/RokidVoiceRelay/tree/codex/aiui-voice-relay-0.9). Source and build history are retained there. No APK installation, real account messaging or glasses deployment was performed during automated verification.
